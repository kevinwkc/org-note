...
*** What is a Cluster?
A set of nodes (actor systems) about which all member are in agreement. Those nodes can then collaborate on a common task.
...
class ClusterReceptionist extends Actor {
  ...
  val awaitingMembers: Receive = {
    case current: ClusterEvent.CurrentClusterState =>
      val address = current.members.toVector map (_.address)
    ...
    case Get(url) => sender ! Failed(url, "no node available")
  }
...
def receive = {
  case ClusterEvent.MemberUp(member) =>
    ...
    case ActorIdentity("42", Some(ref)) => context.watch(self)
...
- need to employ suitable data structures, for example CRDTs (Commutative Replicated Data Types)
...
The structure may change over time defined by a protocol.

Superficially current Actor implementations are unityped:
...
- allow dynamic composition of actor systems
*** Interceptors
#+begin_src scala
  ...
  def receive = {
    case Get(email) =>
      (userService ? FindByEmail(email)).mapTo[UserInfo]
        .map(info => Result(info.post.filter(_.email == email)))
...
class PostSummary(...) extends Actor {
  implicit val timout = Timeout(500.millis)
  def receive = {
    case Get(postId, user, password) =>
      val response = for {
        status <- (publisher ? GetStatus(postId)).mapTo[PostStatus]
        text   <- (postStore ? Get(postId)).mapTp[Post]
        auth   <- (authService ? Login(user, password)).mapTp[AuthStatus] 
      } yield
        if (auth.successful) Result(status, text)
        else Failure("not authroized")
...
*** Round-Robin Routing
- equal distribution of messages to routes
...
- steering the routing weights subject to feedback control theory
  ...
  - over-dampending
*** Random Routing
- asymptotically equal distribution of messages to routes
...
*** Consistent Hashing
- splitting incoming message stream accordint to some criterion
- bundle substreams and send them to the same routees consistenly
- can exhibit systematic imbalances based on hashing function
- different latencies for parts of the input spectrum
- no shared state necessart between different routers to the same targets
*** Replication of Stateful Actors
Consistent Hashing can be used if substreams correspond to indepedent parts of the state.

Multiple writers to the same state require appropriate data structures and are eventually consistent.
*** Replication of Stateful Actors (what gui?)
...
#+begin_src scala
class PostSummary(...) extends Actor {
  implicit val timeout = Timout(500.millis)
  def receive = {
    case Get(postId, user, password) =>
      val status = (publisher ? GetStatus(postId)).mapTo[PostStatus]
      val text = (postStore ? Get(postId)).mapTp[Post]
